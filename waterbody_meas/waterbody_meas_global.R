...
get_waterbody_meas = function(pool, survey_id) {
  qry = glue("select wb.waterbody_measurement_id, wc.clarity_type_short_description as clarity_type, ",
             ...
             "from spawning_ground.waterbody_measurement as wb ",
             "inner join spawning_ground.survey as s on wb.survey_id = s.survey_id ",
...
waterbody_meas_insert = function(pool, new_wbm_values) {
  ...
  insert_result = dbSendStatement(
    con, glue_sql("INSERT INTO spawning_ground.waterbody_measurement (",
                  "survey_id, ",
...
waterbody_meas_update = function(pool, waterbody_meas_edit_values) {
  ...
  update_result = dbSendStatement(
    con, glue_sql("UPDATE spawning_ground.waterbody_measurement SET ",
...
waterbody_meas_delete = function(pool, delete_values) {
  ...
  delete_result = dbSendStatement(
    con, glue_sql("DELETE FROM spawning_ground.waterbody_measurement WHERE waterbody_measurement_id = $1"))
