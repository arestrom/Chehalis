...
get_individual_fish = function(fish_encounter_id) {
  qry = glue("select ind.individual_fish_id, ",
             ...
             "from spawning_ground.individual_fish as ind ",
             "left join spawning_ground.fish_condition_type_lut as fc on ind.fish_condition_type_id = fc.fish_condition_type_id ",
             "left join spawning_ground.fish_trauma_type_lut as ft on ind.fish_trauma_type_id = ft.fish_trauma_type_id ",
             "left join spawning_ground.gill_condition_type_lut as gcn on ind.gill_condition_type_id = gcn.gill_condition_type_id ",
             "left join spawning_ground.spawn_condition_type_lut as sc on ind.spawn_condition_type_id = sc.spawn_condition_type_id ",
             "left join spawning_ground.cwt_result_type_lut as cr on ind.cwt_result_type_id = cr.cwt_result_type_id ",
             "left join spawning_ground.age_code_lut as ac on ind.age_code_id = ac.age_code_id ",
...
individual_fish_insert = function(new_individual_fish_values) {
  ...
  insert_result = dbSendStatement(
    con, glue_sql("INSERT INTO spawning_ground.individual_fish (",
                  "fish_encounter_id, ",
...
individual_fish_update = function(individual_fish_edit_values) {
  ...
  update_result = dbSendStatement(
    con, glue_sql("UPDATE spawning_ground.individual_fish SET ",
...
individual_fish_delete = function(delete_values) {
  ...
  delete_result = dbSendStatement(
    con, glue_sql("DELETE FROM spawning_ground.individual_fish WHERE individual_fish_id = $1"))
