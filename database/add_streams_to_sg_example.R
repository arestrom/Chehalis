#===========================================================================
# Example code to add new streams to SG.
#
# Notes:
#  1.
#
...
#=========================================================================
# Import all existing stream data from sg for WRIAs 22 and 23
#=========================================================================
...
#=========================================================================
# Update any missing LLID to DB....inspect in QGIS to find LLID.
#=========================================================================
...
#=========================================================================
# Get most recent set of stream catalog code data
#=========================================================================

# # Get latest gdb...as of 2020-03-12
...
# Get data
cat_llid_chehalis = read_sf("cat_llid_chehalis_2020-03-12.gpkg",
                            layer = "cat_llid_chehalis_2020-03-12", crs = 2927)

#=========================================================================
# Get most recent set of LLID data
#=========================================================================

# # Get latest llid data....see notes above....but this time from a gdb
...
llid_chehalis = read_sf("llid_chehalis_2020-03-12.gpkg",
...
#======== Unnamed trib to EF Hoquiam R =============================

# Check for previously assigned surveys: Result:

# Set stream llid....from QGIS...latest LLID data
ll_id = '1238416470524'
...
wb = tibble(waterbody_id = remisc::get_uuid(1L),
            waterbody_name = "Unnamed trib to EF Hoquiam R at RM 7.0",
            waterbody_display_name = "Unnamed trib to EF Hoquiam R at RM 7.0",
...
# Get info from cat_llid....Only 0508 has data in SGS
st_cat = cat_llid_chehalis %>%
  filter(llid == ll_id | stringi::stri_detect_fixed(cat_name, "0508", max_count = 1))
...
# Check if the waterbody_id already exists in stream table
wb_id_2 = streams_st %>%
  filter(stringi::stri_detect_fixed(waterbody_display_name, "0146", max_count = 1))

# Check if the waterbody_id already exists in stream table
wb_id_3 = streams_st %>%
  filter(stringi::stri_detect_fixed(cat_code, "22.0146", max_count = 1))

# #============ Manually inspect in QGIS, then enter new stream data here if needed =====================
#
# # Create new entry in waterbody_lut
# wb = tibble(waterbody_id = remisc::get_uuid(1L),
#             waterbody_name = "Tributary 0923",
#             waterbody_display_name = "Tributary (23.0923)",
#             latitude_longitude_id = ll_id,
#             stream_catalog_code = "23.0923",
#             tributary_to_name = "SF Newaukum River",
#             obsolete_flag = 0L,
#             obsolete_datetime = as.POSIXct(NA))
#
# # Write to local
...
# Update waterbody_lut with llid to local
qry = glue("UPDATE waterbody_lut ",
           "SET latitude_longitude_id = '{ll_id}', ",
           "waterbody_name = 'Trib 0146', ",
           "waterbody_display_name = 'Trib 0146 (22.0146)', ",
           "tributary_to_name = 'EF Hoquiam River' ",
...
# Update waterbody_lut with llid to prod
