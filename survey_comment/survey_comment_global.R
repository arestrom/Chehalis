...
get_survey_comment = function(pool, survey_id) {
  qry = glue("select sc.survey_comment_id, ars.area_surveyed, ",
             ...
             "sc.modified_by ",
             "from spawning_ground.survey_comment as sc ",
             "left join spawning_ground.area_surveyed_lut as ars on sc.area_surveyed_id = ars.area_surveyed_id ",
             "left join spawning_ground.fish_abundance_condition_lut as fa on sc.fish_abundance_condition_id = fa.fish_abundance_condition_id ",
             "left join spawning_ground.stream_condition_lut as stc on sc.stream_condition_id = stc.stream_condition_id ",
             "left join spawning_ground.stream_flow_type_lut as sf on sc.stream_flow_type_id = sf.stream_flow_type_id ",
             "left join spawning_ground.survey_count_condition_lut as cc on sc.survey_count_condition_id = cc.survey_count_condition_id ",
             "left join spawning_ground.survey_direction_lut as sd on sc.survey_direction_id = sd.survey_direction_id ",
             "left join spawning_ground.survey_timing_lut as st on sc.survey_timing_id = st.survey_timing_id ",
             "left join spawning_ground.visibility_condition_lut as vc on sc.visibility_condition_id = vc.visibility_condition_id ",
             "left join spawning_ground.visibility_type_lut as vt on sc.visibility_type_id = vt.visibility_type_id ",
             "left join spawning_ground.weather_type_lut as wt on sc.weather_type_id = wt.weather_type_id ",
...
survey_comment_insert = function(pool, new_comment_values) {
  ...
  insert_result = dbSendStatement(
    con, glue_sql("INSERT INTO spawning_ground.survey_comment (",
                  "survey_id, ",
                  "area_surveyed_id, ",
...
survey_comment_update = function(pool, comment_edit_values) {
  ...
  update_result = dbSendStatement(
    con, glue_sql("UPDATE spawning_ground.survey_comment SET ",
                  "area_surveyed_id = $1, ",
...
survey_comment_delete = function(pool, delete_values) {
  ...
  delete_result = dbSendStatement(
    con, glue_sql("DELETE FROM spawning_ground.survey_comment WHERE survey_comment_id = $1"))
