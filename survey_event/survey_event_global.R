...
get_survey_event = function(pool, survey_id) {
  qry = glue("select se.survey_event_id, se.species_id, sp.common_name as species, ",
             ...
             "from spawning_ground.survey_event as se ",
             ...
             "left join spawning_ground.cwt_detection_method_lut as cw on se.cwt_detection_method_id = cw.cwt_detection_method_id ",
...
survey_event_insert = function(pool, new_event_values) {
  ...
  insert_result = dbSendStatement(
    con, glue_sql("INSERT INTO spawning_ground.survey_event (",
                  "survey_id, ",
                  "species_id, ",
...
survey_event_update = function(pool, survey_event_edit_values) {
  ...
  update_result = dbSendStatement(
    con, glue_sql("UPDATE spawning_ground.survey_event SET ",
                  "species_id = $1, ",
...
survey_event_delete = function(pool, delete_values) {
  ...
  delete_result = dbSendStatement(
    con, glue_sql("DELETE FROM spawning_ground.survey_event WHERE survey_event_id = $1"))
