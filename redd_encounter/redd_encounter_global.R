...
redd_encounter_insert = function(pool, new_redd_encounter_values) {
  ...
  insert_result = dbSendStatement(
    con, glue_sql("INSERT INTO spawning_ground.redd_encounter (",
                  "survey_event_id, ",
                  "redd_location_id, ",
                  "redd_status_id, ",
...
redd_encounter_update = function(pool, redd_encounter_edit_values) {
  ...
  update_result = dbSendStatement(
    con, glue_sql("UPDATE spawning_ground.redd_encounter SET ",
                  "redd_location_id = $1, ",
                  "redd_status_id = $2, ",
...
get_redd_encounter_dependencies = function(pool, redd_encounter_id) {
  qry = glue("select ",
             ...
             "from spawning_ground.redd_encounter as rd ",
             "left join spawning_ground.individual_redd as ir on rd.redd_encounter_id = ir.redd_encounter_id ",
             "left join spawning_ground.redd_confidence as rc on rd.redd_encounter_id = rc.redd_encounter_id ",
             "left join spawning_ground.redd_substrate as rs on rd.redd_encounter_id = rs.redd_encounter_id ",
...
redd_encounter_delete = function(pool, delete_values) {
  ...
  delete_result = dbSendStatement(
    con, glue_sql("DELETE FROM spawning_ground.redd_encounter WHERE redd_encounter_id = $1"))
