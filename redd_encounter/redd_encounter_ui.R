...
redd_encounter_ui = tags$div(
  ...
  tippy("<i style='color:#1a5e86;padding-left:8px', class='fas fa-info-circle'></i>",
        tooltip = glue("<span style='font-size:11px;'>",
                       ...
                       "the redd location association, select 'no location data'.<span>")),
  br(),
  br(),
  timeInput(inputId = "redd_encounter_time_select", "Encounter Time", seconds = FALSE),
  uiOutput("redd_status_select", inline = TRUE),
  uiOutput("redd_name_select", inline = TRUE),
  numericInput(inputId = "redd_count_input", label = "Redd Count", value = 0,
               min = 0, step = 1, width = "75px"),
  textAreaInput(inputId = "redd_comment_input", label = "Redd Comment", value = "",
