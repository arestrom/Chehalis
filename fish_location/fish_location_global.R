...
get_new_fish_location = function(pool, waterbody_id) {
  ...
  qry = glue("select floc.location_id as fish_location_id, ",
             ...
             "inner join spawning_ground.stream_channel_type_lut as sc on ",
             "floc.stream_channel_type_id = sc.stream_channel_type_id ",
             "inner join spawning_ground.location_orientation_type_lut as lo on ",
             "floc.location_orientation_type_id = lo.location_orientation_type_id ",
...
get_previous_fish_location = function(pool, waterbody_id, up_rm, lo_rm, survey_date, species_id) {
  ...
  qry = glue("select s.survey_datetime as fish_survey_date, ",
             ...
             "where date(s.survey_datetime::timestamp at time zone 'America/Los_Angeles') > date '{survey_date}' - interval '3 months' ",
             "and date(s.survey_datetime::timestamp at time zone 'America/Los_Angeles') <= date '{survey_date}' ",
             "and floc.waterbody_id = '{waterbody_id}' ",
